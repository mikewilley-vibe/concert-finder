import type { TicketmasterShow } from "./ticketmaster";
...
export async function loadSavedTicketmasterEvents(
  ...
  return (data ?? []).flatMap((raw) => {
    ...
    const show: TicketmasterShow = {
      ...
      venueName: optionalString(row.venue_name) ?? "",
      city: optionalString(row.city) ?? "",
      state: optionalString(row.state) ?? "",
      matchedLabels: [],
    };
    const timeLabel = optionalString(row.time_label);
    const image = optionalString(row.image_url);
    const url = optionalString(row.ticket_url);
