...
export class WatchStateUnavailableError extends Error {
  constructor(message = "New-show inbox is not available yet.") {
    super(message);
    this.name = "WatchStateUnavailableError";
  }
}
...
export async function loadOwnWatchState(supabase: SupabaseClient) {
  const { data, error } = await supabase
    ...
    .select(
      "id, item_label, item_type, new_event_ids, last_checked_at, initialized_at",
    )
    .order("last_checked_at", { ascending: false });

  if (error) {
    throw new WatchStateUnavailableError();
  }
...
export async function markOwnWatchStateSeen(
  ...
  const { error } = await supabase.rpc("mark_ticketmaster_watch_state_seen", {
    target_id: rowId,
  });
